...
// extern void usb_power_ctrl(int onoff);
// extern void wifi_power_ctrl_809(int onoff);
// extern void wifi_power_ctrl(int onoff);
// extern void ipod_power_ctrl(int onoff);
// extern void vcc3v_power_ctrl(int onoff);
 
static void power_led_ctrl(int onoff)
{
        int gpio_handler;
	gpio_handler = gpio_request_ex("lcd0_para", "power_led_ctrl");		
	if(!gpio_handler) {
		pr_err("============%s request power_led_ctrl err==============\n", __FUNCTION__);
		return;
	}
	if(onoff) {
// 		printk("============power led on==============\n");
		gpio_write_one_pin_value(gpio_handler, 1, "power_led_ctrl");		
	} 
	else {
// 		printk("============power led off==============\n");
		gpio_write_one_pin_value(gpio_handler, 0, "power_led_ctrl");		
	}	
}

static void lcd_bl_power_ctrl(int onoff)
{
        int gpio_handler;
	gpio_handler = gpio_request_ex("lcd0_para", "lcd_bl_power_en");		
	if(!gpio_handler) {
		pr_err("============%s request lcd_bl_power_en err==============\n", __FUNCTION__);
		return;
	}
        printk("============lcd bl set to  %d==============\n",onoff);
	gpio_write_one_pin_value(gpio_handler, onoff, "lcd_bl_power_en");	
}

static void lcd_bl_en_ctrl(int onoff)
{
        int gpio_handler;
	gpio_handler = gpio_request_ex("lcd0_para", "lcd_bl_en");		
	if(!gpio_handler) {
		pr_err("============%s request lcd bl en err==============\n", __FUNCTION__);
		return;
	}
        printk("============lcd bl en  %d==============\n",onoff);
	gpio_write_one_pin_value(gpio_handler, onoff, "lcd_bl_en");	
}
...
int gpio_init(void)
{
    ...
    ret = script_parser_init((char *)(sys_cofig_data));
    #else
    ret = script_parser_init((char *)__va(SYS_CONFIG_MEMBASE));
    #endif
    power_led_ctrl(1);
    lcd_bl_power_ctrl(1);
    lcd_bl_en_ctrl(1);
